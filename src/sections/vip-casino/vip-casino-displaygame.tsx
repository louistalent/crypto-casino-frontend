...
const CasinoGameList = () => {
    ...
    const { gameSearchName, gameURLset, playURL } = useGameContext();
    ...
    const init = useCallback(async () => {
        if (providerName === undefined || providerName === '') {
            const res = await gameget('casinovip');
            if (res.data) {
    ...
    const search = useCallback(async () => {
        if (gameSearchName !== '') {
            const res = await searchGameName({ category: 'casinovip', name: gameSearchName });
            setProviderGameList(res.data);
        } else {
            init();
        }
